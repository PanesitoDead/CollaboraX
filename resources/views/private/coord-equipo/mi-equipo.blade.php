                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($invitacion->fecha_expiracion)->format('d/m/Y H:i') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
...
<div id="invitarColaboradorModal" class="fixed inset-0 hidden z-50" role="dialog" aria-modal="true">
  <div class="fixed inset-0 bg-black/50" onclick="closeInvitarModal()"></div>
        ...
        <div class="px-6 py-4 space-y-4">
          ...
          <div id="selected-colaboradores" class="hidden">
            ...
            <div id="selected-list"
                 class="flex flex-wrap gap-2 p-2 border rounded-md min-h-[40px]">
          ...
          <div>
            ...
            <div class="border rounded-md max-h-48 overflow-y-auto">
                @forelse($colaboradores_disponibles as $colaborador)
                    <div class="colaborador-item flex items-center justify-between p-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                        ...
                        data-nombre="{{ $colaborador->nombre_completo }}"
                        data-email="{{ $colaborador->usuario->correo }}"
                        data-rol="{{ $colaborador->usuario->rol->nombre }}">
                        <div class="flex items-center">
                        <div class="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center mr-3">
                            <span class="text-sm font-medium">{{ substr($colaborador->nombre_completo, 0, 2) }}</span>
                        </div>
                        <div>
                            <div class="text-sm font-medium text-gray-900">{{ $colaborador->nombre_completo }}</div>
                            <div class="text-xs text-gray-500">{{ $colaborador->usuario->correo }}</div>
                        </div>
                        </div>
                        <div class="flex items-center gap-2">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                            {{ $colaborador->usuario->rol->nombre }}
                        </span>
...
<div id="programarReunionModal" class="fixed inset-0 hidden z-50" role="dialog" aria-modal="true">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {        
        ...
        function closeInvitarModal() {
            document.getElementById('invitarColaboradorModal').classList.add('hidden');
        }

        function closeReunionModal() {
            document.getElementById('programarReunionModal').classList.add('hidden');
        }
            ...
            if (selectedColaboradores.length > 0) {
                ...
                document.querySelectorAll('.remove-colaborador').forEach(btn => {
                    btn.addEventListener('click', function() {
                        ...
                        const item = document.querySelector(`.colaborador-item[data-id="${id}"]`);
                        if (item) {
                            item.classList.remove('bg-blue-50');
                            const check = item.querySelector('.colaborador-check');
                            if (check) {
