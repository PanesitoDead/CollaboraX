        ...
        <div class="text-center">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
                ...
                <div class="text-sm text-gray-600">
                    ...
                    <p><strong>Rol:</strong> {{ auth()->user()->rol->nombre ?? 'Sin rol asignado' }}</p>
                </div>
            </div>
            @endauth
            ...
            <div class="space-y-3">
                ...
                @auth
                    <!-- Botón para ir al dashboard según el rol -->
                    @if(auth()->user()->rol)
                        @php
                            use Illuminate\Support\Facades\Auth;
                            $user = Auth::user();
                            $rolNombre = $user->rol ? $user->rol->nombre : 'Sin rol';
                            $dashboardRoute = match($rolNombre) {
                                'Super Admin' => route('super-admin.dashboard'),
                                'Admin' => route('admin.dashboard.index'),
                                'Coord. General' => route('coordinador-general.dashboard'),
                                'Coord. Equipo' => route('coord-equipo.dashboard'),
                                'Colaborador' => route('colaborador.actividades'),
                                default => route('home')
                            };
                        @endphp
